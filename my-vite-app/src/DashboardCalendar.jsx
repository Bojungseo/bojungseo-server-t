...
function DashboardCalendar() {
  ...
  useEffect(() => {
    ...
    setCurrentYearMonth(`${today.getFullYear()}년 ${today.getMonth() + 1}월`);
  }, []);
    ...
    const fetchHolidays = async () => {
      ...
      try {
        const res = await axios.get(
          ...
          {
            params: {
              ServiceKey: "c7745b47339ea22a7069fa3dae3aff8930f65da92c6dddf1e9e1a5948cba605c", // 공공데이터포털 키
              solYear: year,
              _type: "json"
            }
          }
        ...
        const holidayList = items.map(h => ({
          id: `holiday-${h.locdate}`,
          title: h.dateName,
          start: h.locdate.toString(),
          backgroundColor: "#FF4444",
          borderColor: "#FF4444",
          allDay: true
        }));
        setHolidayEvents(holidayList);
      } catch (err) {
  ...
  const updateYearMonth = () => {
    ...
    setCurrentYearMonth(`${date.getFullYear()}년 ${date.getMonth() + 1}월`);
  };
    ...
    <div className="bg-white p-4 rounded shadow">
      ...
      <FullCalendar
        ...
        dayCellContent={(arg) => {
          ...
          return { html: `<span style="color:${color};">${arg.dayNumberText}</span>` };
        }}
