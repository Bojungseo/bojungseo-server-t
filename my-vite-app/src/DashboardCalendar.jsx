...
async function fetchSpecialDays(year, month) {
  const allEvents = [];
  for (const service of API_SERVICES) {
    try {
      const url = `https://apis.data.go.kr/B090041/openapi/service/SpcdeInfoService/${service.key}?solYear=${year}&solMonth=${String(month).padStart(2,"0")}&ServiceKey=${HOLIDAY_API_KEY}&_type=json`;
      ...
      items.forEach(h => {
        ...
        allEvents.push({
          title: `${h.dateName} (${service.label})`,
          start: dateStr,
          allDay: true,
          backgroundColor: service.color,
          borderColor: service.color,
          color: service.color,
          id: `${service.key}-${h.locdate}`,
          extendedProps: { isSpecialDay: true }
        });
      });
    } catch (err) {
...
function DashboardCalendar() {
  ...
  const [holidayEvents, setHolidayEvents] = useState([]); // 특일/공휴일 이벤트
  ...
  const updateYearMonth = async () => {
    ...
    const specialEvents = await fetchSpecialDays(y, m);
    setHolidayEvents(specialEvents);
  };
  ...
  const handleEventDrop = async (info) => {
    if (info.event.extendedProps.isSpecialDay) {
      info.revert(); // 특일 이벤트는 이동 불가
      return;
    }
    try {
  ...
  const renderAddButton = () => (
    <button className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 whitespace-nowrap" onClick={() => { setModalData({ id: null, title: "", content: "", date: "", color: DEFAULT_COLORS[0] }); setCustomColor(""); setModalOpen(true); }}>
      일정 추가
    </button>
  );
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow w-96 max-h-[80vh] overflow-y-auto">
            ...
            {!modalData.id && <div className="mb-3"><label className="mr-2">날짜:</label><input type="date" className="border p-2 rounded" value={modalData.date} onChange={(e) => setModalData({ ...modalData, date: e.target.value })} /></div>}
            ...
            <div className="mb-3"><span className="mr-2">직접 선택:</span><input type="color" className="w-10 h-8" value={customColor} onChange={(e) => setCustomColor(e.target.value)} /></div>
          ...
          <div className="bg-white p-6 rounded shadow w-96 max-h-[80vh] overflow-y-auto">
            ...
            <ul className="space-y-2 mb-4">
              ...
              {eventsForSelectedDate.map((e) => (
                <li key={e.id} className="p-2 rounded cursor-pointer text-white" style={{ backgroundColor: e.color }} onClick={() => { setModalData({ id: e.id, title: e.title, content: e.content || "", date: e.start, color: e.color }); setModalOpen(true); setDateListModalOpen(false); }}>{e.title}</li>
              ))}
            </ul>
            <div className="flex justify-end gap-2">
